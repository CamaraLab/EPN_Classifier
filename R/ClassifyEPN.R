...
ClassifyEPN <- function(bulk, permutations = 10000){
  ...
  cat(" ",paste0(round(length(gene_set$ST_SE)/50,2)*100,"% of the EPN molecular subgroup marker genes are expressed in your bulk data"))

  if (length(gene_set[[1]]) <= 5){
    message("5 or fewer EPN molecular subgroup marker genes are expressed in your data. This classification might be inaccurate")
  }
  if (length(gene_set[[1]]) == 0){
    message("None of the EPN moleular subgroup marker genes are expressed in your data. Cannot classify data")
    return(NULL)
  }
